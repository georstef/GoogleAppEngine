...
def create_virtualenv(is_windows):
  if not os.path.exists(FILE_VENV):
    os.system('%s/virtualenv.exe --no-site-packages %s' % (PATH_TO_PYTHON27_SCRIPTS, DIR_VENV))
    os.system('echo %s >> %s' % (
...
def install_py_libs():
  ...
  exec_pip_commands('%s/pip install -q -t %s -r %s' % (PATH_TO_PYTHON27_SCRIPTS, PATH_TO_INSTALL_REQUIREMENTS, FILE_LIB_REQUIREMENTS))
...
def run_start():
  ...
  port = int(ARGS.port)
  run_command = '''
      %s/python.exe %s/dev_appserver.py %s
      --host %s
      --port %s
      --admin_port %s
      --storage_path=%s
      ...
      %s
    ''' % (PATH_TO_PYTHON27, PATH_TO_GOOGLE_APPENGINE, DIR_MAIN, ARGS.host, port, port + 1, DIR_STORAGE, clear,
           " ".join(ARGS.args))
